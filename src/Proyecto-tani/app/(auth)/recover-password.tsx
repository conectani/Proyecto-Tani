...
export default function RecoverPasswordScreen() {
  ...
  const handleRecover = async () => {
    if (!identificacion.trim()) {
      Alert.alert('Campo Obligatorio', 'Por favor ingresa tu DNI o correo electrónico.');
      return;
    }

    try {
      let email = identificacion.trim();
      if (!email.includes('@')) {
        // Si no contiene un @, asumimos que es un DNI e intentamos usar el formato dni@tani.app
        email = `${email}@tani.app`;
      }
      ...
      Alert.alert(
        'Enlace Enviado',
        `Si tu cuenta existe, se ha enviado un correo de recuperación a ${email}.`,
        [{ text: 'Aceptar', onPress: () => router.back() }]
      );
    } catch (e: any) {
      Alert.alert('Error', e.message || 'Ocurrió un error al enviar el correo de recuperación.');
    }
  };
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.scroll} bounces={false}>
        ...
        <View style={styles.card}>
          ...
          <TouchableOpacity style={styles.submitBtn} activeOpacity={0.85} onPress={handleRecover}>
